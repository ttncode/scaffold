...
# under, read back from the manifest mise.root.toml wrote rather than
# recomputed from the owner and name, so `scaffold add` in month six lands
# under the same path as the first application did.
project_image_base() {
  local value
  value="$(yq -p toml -oy -r '.vars.image // ""' "${1}/mise.toml" 2>/dev/null || true)"
  [ -n "$value" ] && [ "$value" != "null" ] \
    || die "no [vars] image in ${1}/mise.toml — was this project generated by an older scaffold?"
  printf '%s' "$value"
}
