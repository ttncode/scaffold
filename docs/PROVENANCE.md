...
| `common/renovate.json` | `renovate.json` | verbatim | byte-identical, but it still extends immich's own internal renovate config and references paths and packages (`mobile/**`, `machine-learning/**`, `ghcr.io/immich-app/*`) that do not exist in a generated project — the same defect `.gitattributes` had before it was reversed below. Not corrected here: task 12 is provenance tracking, not fixing what it finds. Flagged as a concern in the task 12 report. |
...
| `adapters/laravel-api/mise.toml`, `adapters/laravel-inertia/mise.toml` | — | original | immich has no PHP stack, so there is no upstream file to adapt. Task names still follow the ADR-0011 vocabulary, but that is a shared naming convention, not a derived file. |
| `adapters/*/Dockerfile`, `adapters/*/.env.example`, `adapters/*/adapter.env`, `adapters/*/lefthook.fragment.yml` | — | original | per-stack overlay files invoking each framework's own generator (ADR-0003), not vendored from immich's own `Dockerfile`/`docker/example.env` — checked against both, no meaningful resemblance beyond both being Dockerfiles. |
| `scaffold`, `lib/*.sh` | — | original | immich has no scaffolding tool. |

## Out of scope, checked and rejected as rows

- `common/renovate.json`'s `extends` target (`immich-app/.github:renovate-config`)
  is itself a third repository this toolbox has never cloned; the row above
  documents the file as shipped, not that dependency.
